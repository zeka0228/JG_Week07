...
int mm_init(void)
{
    ...
    PUT(heap_listp + (4*WSIZE), PACK(0, 1)); //prologue Footer
    ...
    PUT(heap_listp + (2*WSIZE), heap_listp + WSIZE); //Prologue Successor = 리스트의 좌측 끝
    PUT(heap_listp + (3*WSIZE), heap_listp + (6*WSIZE)); //Prologue Predeccessor = 리스트의 우측 끝
